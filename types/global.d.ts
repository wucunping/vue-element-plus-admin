...
declare global {
	...
	// 声明一个类型 Recordable，允许传入两个类型参数 T 和 K，默认值为 any 和 string，然后返回一个对象类型，该对象的键为 K 类型，值为 T 类型
	declare type Recordable<T = any, K = string> = Record<K extends null | undefined ? string : K, T>

	// 声明一个类型 RemoveReadonly，允许传入一个类型参数 T，然后返回一个对象类型，该对象类型的所有属性都不是只读的
	...
	// 声明一个类型 AxiosContentType，允许为字符串类型，值为 'application/json'、'application/x-www-form-urlencoded'、'multipart/form-data' 或 'text/plain'
